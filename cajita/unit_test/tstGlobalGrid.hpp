...
void sparseGridTest3d()
{
    ...
    auto global_grid = createGlobalGrid( partitioner, MPI_COMM_WORLD,
                                         global_mesh, is_dim_periodic );
    ...
    global_grid->computeNumCellAndOffsetFromTilePartition( part,
                                                           cell_per_tile_dim );
    std::array<int, 3> owned_cells_gt = {
        ( part[0][cart_rank[0] + 1] - part[0][cart_rank[0]] ) *
            cell_per_tile_dim,
        ( part[1][cart_rank[1] + 1] - part[1][cart_rank[1]] ) *
            cell_per_tile_dim,
        ( part[2][cart_rank[2] + 1] - part[2][cart_rank[2]] ) *
            cell_per_tile_dim };

    for ( int d = 0; d < 3; ++d )
    {
        EXPECT_EQ( global_grid->ownedNumCell( d ), owned_cells_gt[d] );
        EXPECT_EQ( global_grid->globalOffset( d ),
                   part[d][cart_rank[d]] * cell_per_tile_dim );
    }
}
