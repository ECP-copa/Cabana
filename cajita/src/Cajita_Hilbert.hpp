...
namespace Kokkos
{
    struct LayoutHilbert2D {
        ...
        explicit constexpr LayoutHilbert2D( size_t N0 = 0, size_t N1 = 0, size_t N2 = 0,
    ...
    namespace Impl
    {
        ...
        struct ViewOffset<Dimension, Kokkos::LayoutHilbert2D, void> {
            ...
            KOKKOS_INLINE_FUNCTION 
            constexpr ViewOffset( std::integral_constant<unsigned, 0> const&, Kokkos::LayoutHilbert2D const& rhs)
        ...
        template <int RankDest, int RankSrc, int CurrentArg, class Arg,
                class... SubViewArgs>
        struct SubviewLegalArgsCompileTime<Kokkos::LayoutHilbert2D, Kokkos::LayoutHilbert2D,
        ...
        struct ViewMapping<
            typename std::enable_if<( std::is_same<typename SrcTraits::specialize, void>::value && ( std::is_same<typename SrcTraits::array_layout, Kokkos::LayoutHilbert2D>::value ) )>::type, SrcTraits, Args...> {
            ...
            private:
                ...
                ( rank <= 2 && R0 && std::is_same<typename SrcTraits::array_layout, Kokkos::LayoutHilbert2D>::value ) ), typename SrcTraits::array_layout, Kokkos::LayoutHilbert2D>::type array_layout;
