...
/*!
  \brief Execute a functor in parallel with a linear execution
  policy specified by the set of given index spaces of views.
  ...
  \param index_spaces The set of view of index spaces over which to loop.

  \param functor The functor to execute. Signature is f(space_id,i,j,k,l)
  space_id is the index of the index space in index_spaces.
 */
template <class FunctorType, class ExecutionSpace>
inline void grid_parallel_for(
    const std::string& label, const ExecutionSpace& exec_space,
    const Kokkos::View<IndexSpace<4>*, typename ExecutionSpace::memory_space>&
        index_spaces,
    const FunctorType& functor )
{
    Kokkos::Profiling::ScopedRegion region( "Cabana::Grid::grid_parallel_for" );
    ...
    Kokkos::View<int*, memory_space> exclusive_offsets( "exclusive_offsets",
                                                        NumSpace );
    Kokkos::View<int*, memory_space> inclusive_offsets( "inclusive_offsets",
                                                        NumSpace );

    auto exclusive_host = Kokkos::create_mirror_view_and_copy(
        Kokkos::HostSpace(), exclusive_offsets );
    auto inclusive_host = Kokkos::create_mirror_view_and_copy(
        Kokkos::HostSpace(), inclusive_offsets );
...
inline void grid_parallel_for(
    const std::string& label, const ExecutionSpace& exec_space,
    const Kokkos::View<IndexSpace<3>*, typename ExecutionSpace::memory_space>&
        index_spaces,
    const FunctorType& functor )
{
    Kokkos::Profiling::ScopedRegion region( "Cabana::Grid::grid_parallel_for" );
...
inline void grid_parallel_for(
    const std::string& label, const ExecutionSpace& exec_space,
    const Kokkos::View<IndexSpace<2>*, typename ExecutionSpace::memory_space>&
        index_spaces,
    const FunctorType& functor )
{
    Kokkos::Profiling::ScopedRegion region( "Cabana::Grid::grid_parallel_for" );
