...
void interpolationSliceTest()
{
    ...
    // Create particles.
    using member_types =
        Cabana::MemberTypes<double[2][2], double[2], double[2], double>;
    Cabana::AoSoA<member_types, TEST_MEMSPACE> particles( "particles",
                                                          num_point );
...
// RUN TESTS
//---------------------------------------------------------------------------//
TEST( Interpolation, View2d ) { interpolationViewTest(); }

TEST( Interpolation, Slice2d ) { interpolationSliceTest(); }
