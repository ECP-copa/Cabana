                        ...
                        const std::array<bool, 2>& is_dim_periodic )
{
          ...
          ++halo_width )
    {
        ...
        // Create a halo.
        auto halo = createStreamHalo( NodeHaloPattern<2>(), halo_width, *array );

        // Gather into the ghosts.
        halo->enqueueGather( TEST_EXECSPACE(), *array );
          ...
          ++halo_width )
    {
        ...
        // Create a multihalo.
        auto halo = createStreamHalo( NodeHaloPattern<2>(), halo_width, *cell_array,
                                *node_array, *face_i_array, *face_j_array );

        // Gather into the ghosts.
        halo->enqueueGather( TEST_EXECSPACE(), *cell_array, *node_array, *face_i_array,
			     *face_j_array );
        ...
        // Scatter from the ghosts back to owned.
        halo->enqueueScatter( TEST_EXECSPACE(), ScatterReduce::Sum(), *cell_array,
                       *node_array, *face_i_array, *face_j_array );
...
void scatterReduceTest( const ReduceFunc& reduce )
{
    ...
    // Create a halo.
    auto halo = createStreamHalo( pattern, array_halo_width, *array );

    // Scatter.
    halo->enqueueScatter( TEST_EXECSPACE(), reduce, *array );
