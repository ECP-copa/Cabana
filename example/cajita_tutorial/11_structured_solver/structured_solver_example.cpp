...
void structuredSolverExample()
{
    ...
    auto owned_space = local_mesh->indexSpace( Cajita::Own(), Cajita::Cell(), Cajita::Local() );
    ...
    auto matrix_entry_layout = createArrayLayout( local_mesh, 7, Cajita::Cell() );
    ...
    {
        auto preconditioner = Cajita::createHypreStructuredSolver<double, MemorySpace>(
    ...
    auto lhs_ref = Cajita::createArray<double, MemorySpace>( "lhs_ref", vector_layout );
    Cajita::ArrayOp::assign( *lhs_ref, 0.0, Cajita::Own() );

    auto ref_solver =
        Cajita::createReferenceConjugateGradient<double, MemorySpace>( *vector_layout );
    ref_solver->setMatrixStencil( stencil );
    const auto& ref_entries = ref_solver->getMatrixValues();
    auto matrix_view = ref_entries.view();
    auto global_space = local_mesh->indexSpace( Cajita::Own(), Cajita::Cell(), Cajita::Global() );
    int ncell_i = global_grid->globalNumEntity( Cajita::Cell(), Cajita::Dim::I );
    int ncell_j = global_grid->globalNumEntity( Cajita::Cell(), Cajita::Dim::J );
    int ncell_k = global_grid->globalNumEntity( Cajita::Cell(), Cajita::Dim::K );
    Kokkos::parallel_for(
        "fill_ref_entries",
        createExecutionPolicy( owned_space, ExecutionSpace() ),
        KOKKOS_LAMBDA( const int i, const int j, const int k ) {
            int gi = i + global_space.min( Cajita::Dim::I ) - owned_space.min( Cajita::Dim::I );
            int gj = j + global_space.min( Cajita::Dim::J ) - owned_space.min( Cajita::Dim::J );
            int gk = k + global_space.min( Cajita::Dim::K ) - owned_space.min( Cajita::Dim::K );
    ...
    auto lhs_ref_host = Kokkos::create_mirror_view_and_copy(
        MemorySpace(), lhs_ref->view() );
    for ( int i = owned_space.min( Cajita::Dim::I ); i < owned_space.max( Cajita::Dim::I );
          ++i )
        for ( int j = owned_space.min( Cajita::Dim::J ); j < owned_space.max( Cajita::Dim::J );
              ++j )
