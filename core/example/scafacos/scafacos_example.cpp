...
using ParticleList =
    Cabana::AoSoA<ParticleDataTypes, MemorySpace, VectorLength>;
                          ...
                          parallel_info &info )
{
    ...
    MPI_Exscan( &( info.n_local_particles ), &offset, 1, MPI_INT, MPI_SUM,
                info.comm );
    ...
    for ( int idx = 0; idx < info.n_local_particles; ++idx )
    {
        ...
        int idx_y = ( idx / info.loc_crystal[0] ) % info.loc_crystal[1] +
                    info.off_crystal[1];
        int idx_z = idx / ( info.loc_crystal[0] * info.loc_crystal[1] ) +
                    info.off_crystal[2];
                  ...
                  parallel_info &info, double gap_space )
{
    ...
    // ScaFaCoS handle (contains common parameters, like box size, periodicity,
    // etc.)
    FCS fcs = FCS_NULL;
    // ScaFaCoS result (contains information about function call, e.g. error
    // messages when the call failed)
    FCSResult result;

    // initialize the handle with the chosen method
    result = fcs_init( &fcs, method.c_str(), info.comm );
    if ( !check_result( result, info.rank ) )
        return;

    // prepare the arrays containing positions (pos), charges (q), field values
    // (f) and potentials (pot)
    ...
    // parameter string to set the periodicity and disable the calculation of
    // near field parts from the calling program
    std::string common_parameters =
        "periodicity,1,1,1,offset,0.0,0.0,0.0,near_field_flag,1";

    // set the box size, so that the crystal should be stable in a periodic
    // system
    ...
    result = fcs_run( fcs, info.n_local_particles, pos.data(), q.data(),
                      f.data(), pot.data() );
    if ( !check_result( result, info.rank ) )
        return;
    ...
    if ( info.rank == 0 )
        std::cout << "total potential: " << total_check[3] << " "
                  << "sum of all field values: " << total_check[0] << " "
                  << total_check[1] << " " << total_check[2] << '\n';
}
...
int main( int argc, char *argv[] )
{
    ...
    if ( argc == 4 )
    {
        ...
        // direct  direct solver (does not work for periodic systems, uses
        // 'halo-systems' (expensive!)) ewald   ewald solver fmm     fast
        // multipole methode p3m     particle - particle particle - mesh method
    ...
    else
    {
        ...
        for ( int i = 0; i < space_dim; ++i )
        {
            ...
            info.n_local_particles *= info.loc_crystal[i];
            info.off_crystal[i] =
                ( info.loc[i] <= ( default_crystal_size % info.dim[i] ) )
                    ? ( info.loc[i] * info.loc_crystal[i] )
                    : ( ( default_crystal_size % info.dim[i] ) *
                            ( info.loc_crystal[i] + 1 ) +
                        ( info.loc[i] -
                          ( default_crystal_size % info.dim[i] ) ) *
                            info.loc_crystal[i] );
        }

        exampleMain( default_crystal_size * default_crystal_size *
                         default_crystal_size,
                     default_crystal_size, method, info, default_gap_space );
    }
