...
//   The vector loop is currently inclusive of the ends, so if you ask to
//   operate over particles 15-33 and your vector length is 16, you will operate
//   over particle 1-48.
#if USE_GPU == 1
#define PARTICLE_OP( C_FUNC, F_FUNC )                                          \
    extern "C" int C_FUNC( int sp, int np );                                   \
    extern "C" CABANA_FUNCTION void F_FUNC( local_particle_struct_t *, int,    \
                                            int );                             \
...
#define PARTICLE_OP( C_FUNC, F_FUNC )                                          \
    ...
    {                                                                          \
        ...
        Kokkos::RangePolicy<ExecutionSpace> range_policy_vec( start_vec,       \
                                                              num_vecs );      \
        Kokkos::parallel_for( range_policy_vec, local_lambda, "example_op" );  \
        return 0;                                                              \
    }
...
// Define MISC_OP
// It is convenient to launch parallel_for operations that have nothing to do
// with particles.
...
#define MISC_OP( C_FUNC, F_FUNC )                                              \
    ...
    {                                                                          \
        auto local_lambda = KOKKOS_LAMBDA( const int idx ) { F_FUNC( idx ); }; \
        Kokkos::RangePolicy<ExecutionSpace> range_policy_vec( start_pt - 1,    \
                                                              end_pt );        \
        Kokkos::parallel_for( range_policy_vec, local_lambda, "example_op" );  \
        return 0;                                                              \
...
using ParticleDataTypes =
    Cabana::MemberTypes<double[6], double[3], long long int>;
...
void cabana_initialize() { Kokkos::initialize(); }

void cabana_finalize( void ) { Kokkos::finalize(); }
