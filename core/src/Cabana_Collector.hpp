...
/*!
  \file Cabana_Collector.hpp
  \brief Multi-node particle redistribution
*/
#ifndef CABANA_COLLECTOR_HPP
#define CABANA_COLLECTOR_HPP
                            ...
                            int>::type* = 0 )
{
    ...
    int my_rank = -1;
    MPI_Comm_rank( collector.comm(), &my_rank );

    // Get the number of neighbors.
    int num_n = collector.numNeighbor();
    ...
    std::size_t num_stay =
        ( num_n > 0 && collector.neighborRank( 0 ) == my_rank )
            ? collector.numExport( 0 )
            : 0;
    ...
    // The collector has its own communication space so choose any tag.
    ...
    {
        recv_range.second = recv_range.first + collector.numImport( n );

        if ( ( collector.numImport( n ) > 0 ) &&
             ( collector.neighborRank( n ) != my_rank ) )
        {
            ...
            MPI_Irecv( recv_subview.data(),
                       ...
                       MPI_BYTE, collector.neighborRank( n ), mpi_tag,
                       collector.comm(), &( requests.back() ) );
        }
...
/*!
  ...
  the collector forward communication plan. Multiple AoSoA version.
...
*/
template <class ExecutionSpace, class Collector_t, class AoSoA_t>
void migrate( ExecutionSpace exec_space, const Collector_t& collector,
...
void migrate( const Collector_t& collector, const AoSoA_t& src,
              ...
              typename std::enable_if<( is_collector<Collector_t>::value &&
                                      ...
                                      int>::type* = 0 )
{
    migrate( typename Collector_t::execution_space{}, collector, src, dst );
}

//---------------------------------------------------------------------------//
/*!
  \brief Synchronously migrate data between two different decompositions using
  the collector forward communication plan. Single AoSoA version that will
...
*/
template <class Collector_t, class AoSoA_t>
void migrate( const Collector_t& collector, AoSoA_t& aosoa,
              typename std::enable_if<( is_collector<Collector_t>::value &&
                                        is_aosoa<AoSoA_t>::value ),
                                      int>::type* = 0 )
{
    migrate( typename Collector_t::execution_space{}, collector, aosoa );
}
...
#endif // end CABANA_COLLECTOR_HPP
