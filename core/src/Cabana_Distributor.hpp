                                      ...
                                      int>::type* = 0 )
{
    ...
    if ( aosoa.size() != distributor.exportSize() )
        throw std::runtime_error( "Cabana::migrate (Distributor, in-place): "
                                      ...
                                      int>::type* = 0 )
{
    ...
    if ( src.size() != distributor.exportSize() )
        throw std::runtime_error(
            "Cabana::migrate (Distributor, slice version): Source is the wrong "
            "size for migration!" );
    if ( dst.size() != distributor.totalNumImport() )
        throw std::runtime_error(
            "Cabana::migrate (Distributor, slice version): Destination is the "
